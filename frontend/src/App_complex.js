...
const VenueStaffDashboard = ({ user, appSettings }) => {
  ...
  console.log('VenueStaffDashboard v2.0 - appSettings.tax_rate:', appSettings?.tax_rate);
  console.log('VenueStaffDashboard v2.0 - full appSettings:', appSettings);
  window.DEBUG_TAX_RATE = appSettings?.tax_rate; // Make it globally accessible
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Preferred Delivery Time (Optional)</label>
                      <input
                        type="time"
                        value={deliveryTime}
                      ...
                      />
                      <p className="text-xs text-gray-500 mt-1">Leave empty if no specific time required</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4">
                    <div className="text-right mb-4">
                      ...
                      {appSettings.tax_rate > 0 && (
                        <p className="text-sm text-gray-600">Tax ({(appSettings.tax_rate * 100).toFixed(1)}%): ${(cart.reduce((total, item) => total + (item.unit_price * item.orderQuantity), 0) * appSettings.tax_rate).toFixed(2)}</p>
                      )}
                      {/* DEBUG v2.0: Tax rate is {appSettings.tax_rate} */}
                      <p className="text-xl font-bold">
                        Total: ${(cart.reduce((total, item) => total + (item.unit_price * item.orderQuantity), 0) * (1 + appSettings.tax_rate)).toFixed(2)}
                      </p>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              ...
              <p><strong>Venue:</strong> {editingInvoice.venue_name}</p>
              <p><strong>Delivery Address:</strong> {editingInvoice.delivery_address}</p>
              <p><strong>Order Date:</strong> {new Date(editingInvoice.order_date).toLocaleDateString()}</p>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {editingInvoice.items.map((item, index) => (
                      <tr key={index}>
                        <td className="px-4 py-4 text-sm font-medium text-gray-900">{item.production_item_name}</td>
                        <td className="px-4 py-4 text-sm text-gray-500">{item.quantity}</td>
                        <td className="px-4 py-4 text-sm text-gray-500">{item.unit_of_measure}</td>
                        <td className="px-4 py-4 text-sm text-gray-500">${item.unit_price.toFixed(2)}</td>
                        <td className="px-4 py-4 text-sm text-gray-500">${(item.quantity * item.unit_price).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {/* Custom Items */}
            <div className="mb-6">
              <h3 className="font-semibold mb-3">Custom Items</h3>
              
              {/* Add New Custom Item Form */}
              <div className="mb-4 p-4 bg-blue-50 rounded-lg">
                <h4 className="font-medium mb-3">Add Custom Item</h4>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., Delivery Fee, Extra Charge"
                    />
                  </div>
                  <div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {customItems.map((item) => (
                        <tr key={item.id}>
                          <td className="px-4 py-4 text-sm font-medium text-gray-900">{item.description}</td>
                          <td className="px-4 py-4 text-sm text-gray-500">{item.quantity}</td>
                          <td className="px-4 py-4 text-sm text-gray-500">{item.unit}</td>
                          <td className="px-4 py-4 text-sm text-gray-500">${item.unit_price.toFixed(2)}</td>
                          <td className="px-4 py-4 text-sm text-gray-500">${item.line_total.toFixed(2)}</td>
            ...
            <div className="flex justify-between items-center pt-4 border-t">
              <div>
                <p className="text-sm text-gray-600">Updated totals will be calculated automatically</p>
              </div>
              <div className="flex space-x-3">
                <button
                  onClick={() => {
                    setEditingInvoice(null);
                    fetchOrders(); // Refresh to get updated totals
                  }}
                ...
                <button
                  onClick={() => {
                    exportInvoicePDF(editingInvoice.id, editingInvoice.invoice_number);
                    setEditingInvoice(null);
                  }}
